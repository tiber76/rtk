...
use crate::cmds::git::{diff_cmd, gh_cmd, git, glab_cmd, gt_cmd};
...
enum Commands {
    ...
    Read {
        ...
        level: crate::core::filter::FilterLevel,
        /// Max lines
    ...
    Telemetry {
        #[command(subcommand)]
        command: crate::core::telemetry_cmd::TelemetrySubcommand,
    },
...
fn run_fallback(parse_error: clap::Error, raw_args: &[OsString]) -> Result<i32> {
    ...
    let error_message = crate::core::utils::strip_ansi(&parse_error.to_string());
    ...
    let toml_match = if std::env::var("RTK_NO_TOML").ok().as_deref() == Some("1") {
        ...
        crate::core::toml_filter::find_matching_filter(&lookup_cmd)
    };
    ...
    if let Some(filter) = toml_match {
        ...
        let result = if filter.filter_stderr {
            ...
            crate::core::utils::resolved_command(&args[0])
                .args(&args[1..])
        ...
        match result {
            Ok(output) => {
                ...
                let filtered = crate::core::toml_filter::apply_filter(filter, &combined_raw);
                println!("{}", filtered);
        ...
        let status = crate::core::utils::resolved_command(&args[0])
            .args(&args[1..])
            .stdin(std::process::Stdio::inherit())
...
fn shell_split(input: &str) -> Vec<String> {
    crate::discover::lexer::shell_split(input)
}
...
pub fn run_from_args(args: impl IntoIterator<Item = impl Into<OsString>>) -> Result<i32> {
    let args: Vec<OsString> = args.into_iter().map(Into::into).collect();
    run_cli_from(args)
}

pub fn oss_command_names() -> Vec<String> {
...
fn run_cli_from(args: Vec<OsString>) -> Result<i32> {
    // Fire-and-forget telemetry ping (1/day, non-blocking)
    crate::core::telemetry::maybe_ping();
    ...
    if !matches!(cli.command, Commands::Gain { .. }) {
        crate::hooks::hook_check::maybe_warn();
    }
    ...
    if is_operational_command(&cli.command) {
        crate::hooks::integrity::runtime_check()?;
    }
    ...
    let code = match cli.command {
        ...
        Commands::Init {
            ...
            if show {
                ...
                crate::hooks::init::uninstall(global, gemini, codex, cursor, cli.verbose)?;
            } else if gemini {
                let patch_mode = if auto_patch {
                    crate::hooks::init::PatchMode::Auto
                } else if no_patch {
                    crate::hooks::init::PatchMode::Skip
                } else {
                    crate::hooks::init::PatchMode::Ask
                };
                crate::hooks::init::run_gemini(global, hook_only, patch_mode, cli.verbose)?;
            } else if copilot {
                crate::hooks::init::run_copilot(cli.verbose)?;
            } else if agent == Some(AgentTarget::Kilocode) {
                if global {
                    anyhow::bail!("Kilo Code is project-scoped. Use: rtk init --agent kilocode");
                }
                crate::hooks::init::run_kilocode_mode(cli.verbose)?;
                ...
                crate::hooks::init::run_antigravity_mode(cli.verbose)?;
            } else {
        ...
        Commands::CcEconomics {
            ...
            crate::analytics::cc_economics::run(daily, weekly, monthly, all, &format, cli.verbose)?;
            0
        }

        Commands::Config { create } => {
            if create {
                let path = crate::core::config::Config::create_default()?;
                println!("Created: {}", path.display());
            } else {
                crate::core::config::show_config()?;
            }
            0
        }

        Commands::Vitest { ref args } => {
            vitest_cmd::run_test(vitest_cmd::JsTestFramework::Vitest, args, cli.verbose)?
        }
        Commands::Jest { ref args } => {
            vitest_cmd::run_test(vitest_cmd::JsTestFramework::Jest, args, cli.verbose)?
        }
        ...
        Commands::Discover {
            ...
            crate::discover::run(project.as_deref(), all, since, limit, &format, cli.verbose)?;
            0
        }

        Commands::Session {} => {
            crate::analytics::session_cmd::run(cli.verbose)?;
            0
        }

        Commands::Telemetry { command } => {
            crate::core::telemetry_cmd::run(&command)?;
            0
        }
                    ...
                    if args.len() > 1 {
                        ...
                        match args[1].as_str() {
                            ...
                            _ => {
                                ...
                                let timer = crate::core::tracking::TimedExecution::start();
                                let mut cmd = crate::core::utils::resolved_command("npx");
                                for arg in &args {
                                    cmd.arg(arg);
                                }
                                ...
                                crate::core::utils::exit_code_from_status(&status, "npx prisma")
                            }
                        }
                    } else {
                        let timer = crate::core::tracking::TimedExecution::start();
                        let status = crate::core::utils::resolved_command("npx")
                            .arg("prisma")
                            .status()
        ...
        Commands::HookAudit { since } => {
            crate::hooks::hook_audit_cmd::run(since, cli.verbose)?;
            0
        }

        Commands::Hook { command } => match command {
            HookCommands::Claude => {
                crate::hooks::hook_cmd::run_claude()?;
                0
            }
            HookCommands::Cursor => {
                crate::hooks::hook_cmd::run_cursor()?;
                0
            }
            HookCommands::Gemini => {
                crate::hooks::hook_cmd::run_gemini()?;
                0
            }
            HookCommands::Copilot => {
                crate::hooks::hook_cmd::run_copilot()?;
                0
            }
        ...
        Commands::Rewrite { args } => {
            let cmd = args.join(" ");
            crate::hooks::rewrite_cmd::run(&cmd)?;
            0
        }
        ...
        Commands::Proxy { args } => {
            ...
            let mut child = ChildGuard(Some(
                crate::core::utils::resolved_command(cmd_name.as_ref())
                    .args(&cmd_args)
                    .stdout(Stdio::piped())
                    .stderr(Stdio::piped())
                    .spawn()
        ...
        Commands::Trust { list } => {
            crate::hooks::trust::run_trust(list)?;
            0
        }

        Commands::Untrust => {
            crate::hooks::trust::run_untrust()?;
            0
        }
        ...
        Commands::Verify {
            ...
            if filter.is_some() {
                ...
                crate::hooks::verify_cmd::run(filter, require_all)?;
            } else {
                // Default or --require-all: always run integrity check first
                crate::hooks::integrity::run_verify(cli.verbose)?;
                crate::hooks::verify_cmd::run(None, require_all)?;
            }
            0
        }
    };
